...
describe('translations', () => {
  ...
  describe('keys built from backend enums', () => {
    const cases: [string, string[]][] = [
      ...
      ['testCaseStatus', ['DRAFT', 'IN_REVIEW', 'ACTIVE', 'DEPRECATED']],
      ['testRun.status', ['PLANNED', 'IN_PROGRESS', 'COMPLETED', 'ABORTED']],
      ['testPlan.status', ['OPEN', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED']],
      ['bugReport.status', ['NEW', 'OPEN', 'IN_PROGRESS', 'RESOLVED', 'CLOSED']],
      ['bugReport.resolution', ['FIXED', 'WONT_FIX', 'DUPLICATE', 'CANNOT_REPRODUCE', 'NOT_A_BUG', 'DEFERRED']],
      ['activity.sentences',
        ['CREATED', 'UPDATED', 'DELETED', 'STATUS_CHANGED', 'COMPLETED', 'REOPENED', 'CLONED', 'MOVED']],
      ...['activity.entityTypes', 'activity.objects'].map((prefix): [string, string[]] => [prefix,
