...
public class TestRunWriteTools {
    ...
    public McpDtos.RecordedStepResult recordStepResult(
        ...
        TestResultResponse result = resolve(run, resultId, testCaseId);
        if (result == null) {
        ...
        TestRunStatus runStatus = startIfPlanned(caller, run);
        // Absent actualResult means "leave alone", for the same reason as comment above.
        ...
        List<McpDtos.StepOutcome> steps = new java.util.ArrayList<>(ordered.size());
    ...
    private static List<StepResultResponse> orderedSteps(TestResultResponse result) {
        ...
        return result.stepResults().stream()
                .sorted(java.util.Comparator.comparingInt(StepResultResponse::orderIndex))
                .toList();
    }
    ...
    private static void requireOpen(TestRunResponse run) {
        if (run.status() == TestRunStatus.COMPLETED || run.status() == TestRunStatus.ABORTED) {
